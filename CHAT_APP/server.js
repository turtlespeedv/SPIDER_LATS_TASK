...
function broadcastToWorkspace(workspaceId, message, sender) {
...
wss.on('connection', (ws, req) => {
  ...
  ws.on('message', (data) => {
    ...
    if (messageData.type === 'message' && workspaceId) {
      ...
      // Send message to all clients in the workspace except the sender
      broadcastToWorkspace(workspaceId, JSON.stringify({
        user: messageData.user,
        message: messageData.message,
      }), ws); // Pass the sender to avoid sending the message back to the sender
    }
  });
